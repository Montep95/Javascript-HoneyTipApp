...
//LikePage로 부터 tip, setTip을 추가로 전달받은 것을 반영
export default function LikeCard({content,navigation,tip, setTip}){

    const detail = () => { // detail 함수 구현 (찜 상세페이지 버튼 구현)
      navigation.navigate('DetailPage',{idx:content.idx})
      // navigation 기능을 이용하여 DetailPage의 인덱스 번호로 그 컨텐츠에 접근하고자함
    }

    const remove = async (cidx) => { // remove 함수 구현 (찜 해제 버튼 구현)
      // OS 확인 절차 추가
      let userUniqueId;
